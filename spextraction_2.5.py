...
def spextraction_images(format_file, data_list, map_save_location, wavelength_keyword='OSF', plot_maps=0, map_load=0, IF_scale=0, meancm_path='/prvt/ilio/EZDisturbance/haze/'):
    ...
    for i_wavel in range(0,len(wavel_spxs)):
        ...
        # units of extracted error value will depend on IF_scale turned on or off. 
        max_iflag_y = np.max(np.where(iflag_spxs==1)[0])
        max_iflag_x = np.max(np.where(iflag_spxs==1)[1])
        error_estimate[0,i_wavel] = np.std(spxs[(max_iflag_x+10):(max_iflag_x+40),(max_iflag_y-30):(max_iflag_y+30)])
        # !!! make it an option to plot the regions of error to see if there's any issues w the region chosen
...
#                    '/Users/emmadahl/Desktop/irtf_dps_retrieval/jupiter.spx',\
